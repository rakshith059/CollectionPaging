...
interface CollectionApiService {
    ...
    @Headers(CONTENT_TYPE_APPLICATION_JSON_CHARSET_UTF_8)
    @GET("/api/v1/collections/{mCollectionName}")
    fun getCollectionOnlyStoriesApiService(@Path("mCollectionName") mCollectionName: String, @Query("limit") limit: Int, @Query("offset") offset: Int, @Query("item-type") itemType: String): Flowable<CollectionResponse>
